...
% to pad zeroes in the first row and first column 
binary_image = [];
for i = 1:size_row+1
    for j = 1:size_col+1
        if (i == 1 | j==1)
...
for row = 2:size_row+1
    for col = 2:size_col+1
        if (binary_image(row, col) == 1)
            if (binary_image(row-1,col-1) == 0 && binary_image(row-1,col) == 0 && binary_image(row,col-1) == 0 && binary_image(row-1,col+1) == 0 && binary_image(row,col-1) == 0)
                labels(row, col) = give_label(index_givelabel);
                index_givelabel = index_givelabel + 1;
                value_givelabel = value_givelabel + 1;
                give_label(index_givelabel) = value_givelabel;
            elseif (binary_image(row-1,col) == 1) 
                labels(row, col) = labels(row-1, col);
            elseif (binary_image(row,col-1) == 1)
                labels(row, col) = labels(row, col-1);
            elseif (binary_image(row-1,col-1) == 1)    
...
% unique(labels)
% cmap = [1,0,0; 0,1,1; 0,1,1];
% diff_objects = ind2rgb(labels, cmap);
% subplot(1,2,2), imshow(bw_image_inverted);
% subplot(1,2,1), imshow(diff_objects);
cmap = [0,0,0;1,1,0;0,1,0];
...
%remapped image
figure;
subplot(1,2,1), imshow(bw_image_inverted), title('Binary Image');
subplot(1,2,2), imshow(RGB), title('Eight Adjacency');
